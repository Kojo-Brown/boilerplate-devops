...
export class SecurityHubStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: SecurityHubStackProps = {}) {
    ...
    // ── GuardDuty Detector ───────────────────────────────────────────────────
    if (enableGuardDuty) {
      // CloudTrail management events are a foundational GuardDuty source: they
      // are always analysed and are not exposed as a toggle on
      // AWS::GuardDuty::Detector, so only S3, Kubernetes, and Malware
      // Protection are configurable here.
